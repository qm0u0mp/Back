package com.haneul.basic.filter;
...
import com.haneul.basic.provider.JwtProvider;
...
public class jwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    // 1. 클라이언트가 토큰을 발급받음
    // 2. 인증 토큰을 발급받은 후, 매 요청마다 인증 토큰을 request header의 Authoriszation 필드의 값으로 Bearer
    // 토큰을 포함하여 요청
    @Override
    ...
    private String parseBearerToken(HttpServletRequest request) {
        ...
        boolean hesAuthorization = StringUtils.hasText(authorization);
        if (!hesAuthorization) {
            return null;
        }
